...
FROM game JOIN eteam ON (team1=eteam.id AND coach LIKE '%Santos')


-- 7.List the player for every goal scored in a game where the stadium was 'National Stadium, Warsaw'

SELECT player
FROM goal JOIN game ON (id=matchid AND stadium LIKE '%Warsaw')
...
SELECT stadium, COUNT(player) AS 'goals scored'
...
SELECT matchid, mdate, COUNT(player) AS 'goals scored'
